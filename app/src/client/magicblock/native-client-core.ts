/**
 * Native MagicBlock runtime.
 *
 * This file contains the current MagicBlock runtime behavior used by the
 * active backend. It exists as a transition point while concrete behavior is
 * drained into focused services such as `read-service`, `write-service`,
 * `session-service`, and `provisioning-service`.
 */
import {
  type AccountInfo,
  Connection,
...
import { readBurnerWallet, resetBurnerWallet } from "../wallet";
...
import { MagicBlockRuntimeContext } from "./runtime-context";
import { MagicBlockAccountReader } from "./account-reader";
import {
  MagicBlockProgramRegistry,
  MissingProgramsError,
} from "./program-registry";
import {
  decodeActiveAction as decodeMagicBlockActiveAction,
  decodeEnergy as decodeMagicBlockEnergy,
  decodeInventory as decodeMagicBlockInventory,
  decodePosition as decodeMagicBlockPosition,
  decodePlayerActionStateFromAccounts as decodeMagicBlockPlayerActionStateFromAccounts,
  decodeTileItem as decodeMagicBlockTileItem,
  getActionKind as getMagicBlockActionKind,
  getPlayerActionRevision as getMagicBlockPlayerActionRevision,
  readI64 as readMagicBlockI64,
  readU64 as readMagicBlockU64,
  selectFreshestPlayerActionState as selectFreshestMagicBlockPlayerActionState,
} from "./decoders";
...
export class MagicBlockNativeClientCore {
  private readonly runtime = new MagicBlockRuntimeContext();
  private readonly stateReader = new MagicBlockAccountReader(
    this.baseConnection,
    this.erConnection
  );
  private readonly programRegistry = new MagicBlockProgramRegistry(
    this.baseConnection
  );
  ...
  private get baseConnection() {
    return this.runtime.baseConnection;
  }

  private get erConnection() {
    return this.runtime.erConnection;
  }

  private get wallet() {
    return this.runtime.wallet;
  }

  private set wallet(wallet: Keypair) {
    this.runtime.wallet = wallet;
  }
  ...
  dispose() {
    ...
    for (const timer of [
      this.playerStateSyncTimer,
      this.visiblePlayerSyncTimer,
      this.tradeOfferSyncTimer,
    ]) {
      if (timer !== null) {
    ...
    this.playerStateSyncTimer = null;
    this.visiblePlayerSyncTimer = null;
    this.tradeOfferSyncTimer = null;
    ...
    this.playerAccountSubscriptionKey = null;

    this.playerActionStateListeners.clear();
    this.playerAppearanceListeners.clear();
    this.playerSelectionListeners.clear();
    this.visiblePlayerListeners.clear();
    this.inventoryListeners.clear();
    this.goldBalanceListeners.clear();
    this.tradeOfferListeners.clear();
    this.farmTileListeners.clear();
    this.tileItemListeners.clear();
  }
  ...
  async boot() {
    ...
    // DOM events are attached by MagicBlockControlBinder so runtime boot has no UI binding side effects.
  ...
  async movePlayer(point: GridPoint) {
    try {
      ...
      await this.programRegistry.requireDeployed([
        "position",
        "energy",
        "activeAction",
        "inventory",
  ...
  async performAction(
    ...
    try {
      ...
      const { ApplySystem } = await loadBoltSdk();
      const systemId = this.getWorldTileActionProgram(mode);
      const { entities, extraAccounts } = this.getWorldTileActionAccounts(
        mode,
        player,
        tileTerrain,
        tileFarm,
      ...
      this.hud.setProgramStatus(
        `${this.describeWorldTileActionMode(mode)} ${point.x}, ${point.y}...`
      );
      await this.sendBoltResult(
        await ApplySystem({
          authority: this.wallet.publicKey,
      ...
      this.hud.setProgramStatus(
        `${this.describeWorldTileActionMode(mode)} confirmed.`
      );
      this.renderActionBusy(confirmedState.activeAction);
      this.emitPlayerActionState(confirmedState);
      ...
      this.hud.setProgramStatus(
        message ? `Tile action failed: ${message}` : "Tile action failed."
      );
      return null;
    }
  }
  ...
  private async refreshNetwork() {
    try {
      const [version, programInfos] = await Promise.all([
        ...
        this.programRegistry.fetchProgramInfos(),
      ]);
  ...
  private async syncPlayerState(options: { announceLoaded?: boolean } = {}) {
    ...
    try {
      if (!this.activePlayerNft) {
        console.info(
          "[Open Wilds] player state sync skipped: no active player"
        );
        return;
      }
  ...
  private async syncVisiblePlayers() {
    if (this.visiblePlayerListeners.size === 0 || this.visiblePlayerSyncing) {
      return;
    }
  ...
  private async syncTradeOffers() {
    ...
    try {
      ...
      const relevantOffers = offers
        .map((account): TradeOfferState | null => {
  ...
  private getWorldTileActionProgram(mode: WorldTileActionMode) {
    switch (mode) {
      case "till":
  ...
  private getWorldTileActionAccounts(
    mode: WorldTileActionMode,
    player: PlayerState,
  ...
  private describeWorldTileActionMode(mode: WorldTileActionMode) {
    switch (mode) {
      case "till":
        return "Tilling";
      case "water":
  ...
  private async fetchPlayerActionStateOnEr(
    ...
    return this.stateReader.fetchPlayerActionState(player);
  }
  ...
  private decodePlayerActionStateFromAccounts(
    ...
    return decodeMagicBlockPlayerActionStateFromAccounts(
      [positionAccount, energyAccount, activeActionAccount],
      source
    );
  }
  ...
  private selectFreshestPlayerActionState(
    ...
    const selectedState = selectFreshestMagicBlockPlayerActionState(
      erState,
      baseState
    );

    if (selectedState === baseState && erState && baseState) {
      console.info(
  ...
  private getPlayerActionRevision(state: PlayerActionState) {
    return getMagicBlockPlayerActionRevision(state);
  }
  ...
  private decodeTileItem(
    ...
    return decodeMagicBlockTileItem(data, fallbackPoint);
  }

  private readI64(view: DataView, offset: number) {
    return readMagicBlockI64(view, offset);
  }

  private readU64(view: DataView, offset: number) {
    return readMagicBlockU64(view, offset);
  }
  ...
  private getActionKind(
    ...
    return getMagicBlockActionKind(action, endsAt);
  }
  ...
  syncAgentDelegateInputs(value: string) {
    for (const input of this.agentDelegateInputs()) {
      input.value = value;
    }
  }

  syncAgentSessionTransactionInputs(value: string) {
  ...
  private parseAgentDelegateValue(value: string): PublicKey | null {
    if (!value) {
      return null;
    }
  ...
  async refreshAgentModeStatus() {
    return this.refreshAgentModeStatusForInput(
      this.currentAgentDelegateValue()
    );
  }

  async refreshAgentModeStatusForInput(delegateValue: string) {
    const inputValue = delegateValue.trim();
    ...
    const delegate = this.parseAgentDelegateValue(inputValue);
  ...
  private parsePreparedBoltSessionTransaction(value: string) {
    ...
    try {
      return Transaction.from(Buffer.from(trimmedValue, "base64"));
    } catch {
      throw new Error("Invalid BOLT session transaction");
    }
  }

  private async ensureBoltSession(
    delegate: PublicKey,
    preparedTransactionValue: string
  ) {
  ...
  async grantAgentSession() {
    return this.grantAgentSessionForInput(
      this.currentAgentDelegateValue(),
      this.currentAgentSessionTransactionValue()
    );
  }

  async grantAgentSessionForInput(
    delegateValue: string,
    preparedTransactionValue: string
  ) {
    const delegate = this.parseAgentDelegateValue(delegateValue.trim());
    ...
    try {
      ...
      if (
        ...
        await this.ensureBoltSession(delegate, preparedTransactionValue);
        await this.refreshAgentModeStatusForInput(delegateValue);
        return;
      }
  ...
  async revokeAgentSession() {
    return this.revokeAgentSessionForInput(this.currentAgentDelegateValue());
  }

  async revokeAgentSessionForInput(delegateValue: string) {
    const delegate = this.parseAgentDelegateValue(delegateValue.trim());
  ...
  async selectPlayerNft(mint: PublicKey) {
  ...
  private async sendBoltResult(
    ...
    return this.runtime.sendBoltResult(result, connection, options);
  }
  ...
  private async sendSignedTransaction(
    ...
    return this.runtime.sendSignedTransaction(transaction, connection, signers);
  }

  private async installAnchorProvider(connection: Connection) {
    await this.runtime.installAnchorProvider(connection);
  }
}
