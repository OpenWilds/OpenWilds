...
const fileRoot = path.resolve(
  process.env.OPEN_WILDS_STUDIO_MCP_ROOT ?? process.cwd()
);
const studioUrl =
  process.env.OPEN_WILDS_STUDIO_URL ??
  process.env.VITE_OPEN_WILDS_STUDIO_URL ??
  "http://localhost:5173/studio";
const authStorePath = path.resolve(
  process.env.OPEN_WILDS_STUDIO_MCP_AUTH_FILE ??
    path.join(os.homedir(), ".open-wilds", "studio-mcp-auth.json")
);
...
let authRefreshToken =
  process.env.OPEN_WILDS_STUDIO_REFRESH_TOKEN ??
  process.env.CONVEX_AUTH_REFRESH_TOKEN ??
  "";
...
if (!authToken) {
  ...
  if (storedAuth?.token) {
    ...
    authRefreshToken = storedAuth.refreshToken ?? "";
    authSource = "stored";
  }
}
...
const tools = [
  tool({
    ...
    handler: async () => {
      await refreshAuthIfNeeded({ silent: true });
      ...
      return {
        ...
        authSource,
        hasRefreshToken: Boolean(authRefreshToken),
        browserLoginUrl: buildStudioAuthUrlPreview(),
        authStorePath,
  ...
  tool({
    name: "studio_login_browser",
    title: "Sign in through browser",
    description:
      "Open Studio in a browser, let the user authenticate there, and capture Convex Auth tokens through a localhost callback.",
    inputSchema: objectSchema({
      studioUrl: stringSchema(
        "Optional Studio URL. Defaults to OPEN_WILDS_STUDIO_URL or http://127.0.0.1:5173/studio."
      ),
      timeoutMs: numberSchema("Optional login timeout in milliseconds."),
      openBrowser: booleanSchema("Whether to open the browser automatically."),
    }),
    handler: async (args) => {
      ...
      return {
        ...
        tokenExpiresAt: authToken
          ? new Date((decodeJwt(authToken)?.exp ?? 0) * 1000).toISOString()
          : null,
  ...
  tool({
    ...
    handler: async (args) => {
      ...
      await setAuthTokens(result.tokens, "studio_sign_in", true);
      return {
        signedIn: true,
        currentUser: await currentUserOrNull(),
      };
...
function requireAuthToken() {
  if (!authToken && !authRefreshToken) {
    throw new Error(
      "Studio auth is required. Use studio_login_browser, studio_sign_in, or set OPEN_WILDS_STUDIO_AUTH_TOKEN to a Convex Auth JWT."
    );
  }
}
...
async function loginWithBrowser(args) {
  const timeoutMs = Math.max(
    5_000,
    Math.min(Number(args.timeoutMs ?? 120_000), 10 * 60_000)
  );
  const state = crypto.randomBytes(24).toString("base64url");
  const server = http.createServer();

  await listenCallbackServer(server);

  const address = server.address();
  if (!address || typeof address === "string") {
    server.close();
    throw new Error("Could not start a local browser auth callback server.");
  }

  const tokensPromise = waitForBrowserAuth(server, state, timeoutMs);
  const loginUrl = buildStudioAuthUrl({
    callbackUrl: `http://127.0.0.1:${address.port}/callback`,
    state,
    studioUrl: args.studioUrl,
  });
  const openedBrowser =
    args.openBrowser === false ? false : await openBrowser(loginUrl);

  try {
    const tokens = await tokensPromise;
    await setAuthTokens(tokens, "browser", true);
    return { loginUrl, openedBrowser };
  } finally {
    server.close();
  }
}

async function listenCallbackServer(server) {
  await new Promise((resolve, reject) => {
    const onError = (error) => {
      server.off("listening", onListening);
      reject(
        new Error(
          `Could not start the local Studio auth callback server: ${toErrorMessage(error)}`
        )
      );
    };
    const onListening = () => {
...
function waitForBrowserAuth(server, state, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    const onRequest = async (request, response) => {
      try {
        ...
        if (requestUrl.pathname !== "/callback") {
          await sendCallbackResponse(response, 404, "Not found.");
          return;
        }
        ...
        if (receivedState !== state) {
          await sendCallbackResponse(
            response,
            400,
            authCallbackHtml("Could not verify the login state.")
          );
          finish(new Error("Browser login returned an invalid state."));
          return;
        }
        ...
        if (!token) {
          await sendCallbackResponse(
            response,
            400,
            authCallbackHtml("Studio did not return an auth token.")
          );
          finish(new Error("Browser login did not return an auth token."));
          return;
        }
        ...
        await sendCallbackResponse(
          response,
          200,
          authCallbackHtml("Studio is connected. You can close this tab.")
        );
        finish(null, { token, refreshToken });
      } catch (error) {
        await sendCallbackResponse(
          response,
          500,
          authCallbackHtml(toErrorMessage(error))
        );
        finish(error);
      }
    };
    ...
    timeout = setTimeout(() => {
      finish(new Error("Timed out waiting for browser Studio authentication."));
    }, timeoutMs);
...
async function refreshAuthIfNeeded(options = {}) {
  if (!authRefreshToken) {
    return;
  }
  if (!options.force && authToken && !tokenNeedsRefresh(authToken)) {
    return;
  }
  ...
  try {
    ...
    if (!result?.tokens?.token) {
      throw new Error("Studio auth refresh did not return an auth token.");
    }

    await setAuthTokens(result.tokens, "refresh", true);
  } catch (error) {
    if (!options.silent) {
      throw error;
    }
...
async function setAuthTokens(tokens, source, persist) {
  authToken = tokens.token;
  authRefreshToken = tokens.refreshToken ?? authRefreshToken;
  authSource = source;
...
function createConvexHttpClient() {
  ...
  if (typeof client.setDebug === "function") {
...
function tokenNeedsRefresh(token) {
  ...
  return tokenInfo.exp * 1000 <= Date.now() + 120_000;
}

function buildStudioAuthUrlPreview() {
  return normalizeStudioAuthUrl().toString();
}

function buildStudioAuthUrl(args) {
  const url = normalizeStudioAuthUrl(args.studioUrl);
  url.searchParams.set("callback", args.callbackUrl);
  url.searchParams.set("state", args.state);
  return url.toString();
}

function normalizeStudioAuthUrl(inputUrl = studioUrl) {
  const url = new URL(inputUrl);
  const pathname = url.pathname.replace(/\/$/, "");
  if (!pathname.endsWith("/mcp-auth")) {
    url.pathname = `${pathname || "/studio"}/mcp-auth`;
  }
  return url;
}
...
async function readRequestBody(request) {
  ...
  for await (const chunk of request) {
    ...
    if (size > 128 * 1024) {
      throw new Error("Browser auth callback body is too large.");
    }
    chunks.push(chunk);
  }
...
async function sendCallbackResponse(response, statusCode, body) {
  if (response.headersSent) {
    return;
  }

  await new Promise((resolve) => {
    const contentType = body.trimStart().startsWith("<!doctype html>")
...
function authCallbackHtml(message) {
  return `<!doctype html><html><head><meta charset="utf-8"><title>Open Wilds Studio</title></head><body><main style="font-family:system-ui,sans-serif;max-width:34rem;margin:4rem auto;line-height:1.5"><h1>Open Wilds Studio</h1><p>${escapeHtml(message)}</p></main></body></html>`;
}

function readStoredAuth() {
  try {
    const contents = fs.readFileSync(authStorePath, "utf8");
    const parsed = JSON.parse(contents);
    return parsed.sessions?.[authStoreKey()] ?? null;
  } catch {
    return null;
  }
}
...
async function writeStoredAuth() {
  ...
  let parsed = { version: 1, sessions: {} };
  try {
    parsed = JSON.parse(await fsp.readFile(authStorePath, "utf8"));
  } catch {
    // Missing or invalid auth stores are replaced with a fresh one.
  }

  parsed.version = 1;
  parsed.sessions = parsed.sessions ?? {};
  parsed.sessions[authStoreKey()] = {
    token: authToken,
    refreshToken: authRefreshToken,
  ...
  await fsp.writeFile(authStorePath, JSON.stringify(parsed, null, 2), {
    mode: 0o600,
  });
  await fsp.chmod(authStorePath, 0o600);
}
...
async function clearStoredAuth() {
  try {
    ...
    if (parsed.sessions) {
      delete parsed.sessions[authStoreKey()];
    }
    await fsp.writeFile(authStorePath, JSON.stringify(parsed, null, 2), {
      mode: 0o600,
    });
  } catch {
    // Missing auth stores are already signed out.
  }
}

function authStoreKey() {
  return convexUrl || "default";
}
...
function assertInsideFileRoot(resolvedPath) {
  if (
    resolvedPath !== fileRoot &&
    !resolvedPath.startsWith(`${fileRoot}${path.sep}`)
  ) {
    throw new Error(
      `Path must stay inside OPEN_WILDS_STUDIO_MCP_ROOT or the current working directory unless OPEN_WILDS_STUDIO_MCP_ALLOW_ABSOLUTE_PATHS=1 is set: ${resolvedPath}`
    );
  }
}
